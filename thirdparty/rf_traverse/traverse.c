...
char *node_shape(int type) 
{
    switch (type) {
    case (ACTION): return "ACTION"; break;
    case (ITERATION): return "ITERATION"; break; /* XXX Never happens. */
    case (SELECTION): return "SELECTION"; break;
    case (JOIN): return "JOIN"; break;
    case (BRANCH): return "BRANCH"; break;
    case (RENDEZVOUS): return "RENDEZVOUS"; break;
    case (PROCESS): return "PROCESS"; break;
...
/*
    ...
    case (ITERATION): return "diamond, style=filled"; break; 
...
void insert_iteration_links(Node from, Node to)
{
	...
	if ((node_color_t)p->data == WHITE) {
	    printf("ITERATION: %s, %s ", to->name, p->name);
	}
    }
...
void print_link(Node from, Node to, int weight)
{
    ...
    if (weight != 0) {
	printf("STANDARD_LINK: %s %s ",from->name, to->name );
...
/*	printf("%ld -> %ld [label=\"again\", weight=%d, style=%s, headport=\"ne\", tailport=\"se\"] ;\n", */
	printf("ITERATION: %s %s \n",from->name, to->name); //rename correctly
    }
}
...
void name_node(Node n)
{
    ...
    if (graph_type != XML) {
	printf("NODE_NAME_TYPE:, %s, %s \n", n->name, node_shape(n->type));

	printf("REQUIRES:,%s,", n->name);
    	print_resources((n->requires));
    	printf("\n");

	printf("PROVIDES:,%s,", n->name);
    	print_resources((n->provides));
    	printf("\n");
	if (n->type == PROCESS) {
	   // printf("NAME_NODE_PROCESS LABLE:, \"%s\"]\n", n->name);
		...
		//printf("NAME_NODE:, label=\"%s\"];\n", n->name);
	    }
	} else {
	   // printf("NAME_NODE:, label=\"\"];\n");
	}
    }
}
